...
import { Github, Eye, EyeOff, Loader2, AlertTriangle, CheckCircle2 } from 'lucide-react';
    ...
    try {
      ...
      if (error) {
        ...
        if (user && !user.confirmed_at && user.identities?.length === 0) {
          ...
          setSuccess(true);
          // Redirect after a delay if auto-confirmed
          setTimeout(() => {
            router.push(`/${locale}/problems`);
          }, 2000);
        }
        ...
        <div className="bg-algora-card-bg rounded-xl border border-[rgba(255,255,255,0.08)] p-8">
          ...
          {success ? (
            <div className="text-center py-8">
              <CheckCircle2 className="w-12 h-12 text-algora-green mx-auto mb-4" />
              <p className="text-sm text-algora-text-muted">
                {t('confirmEmail')}
              </p>
            </div>
